...
import { Plus, Trash2, Edit, Eye, EyeOff, Upload, ClipboardList, Download, Calendar } from "lucide-react";

const semesters = [1, 2, 3, 4, 5, 6];

export const AdminAssignmentSection = () => {
    ...
    const togglePublishMutation = useMutation({
        mutationFn: async ({ id, is_published }: { id: string; is_published: boolean }) => {
            const { error } = await supabase.from("assignments").update({ is_published }).eq("id", id);
            if (error) throw error;
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["admin-assignments"] });
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const { data: { publicUrl } } = supabase.storage.from("uploads").getPublicUrl(`assignments/${fileName}`);
